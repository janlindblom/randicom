...
Adafruit_SSD1327 console =
    Adafruit_SSD1327(128, 128, &Wire1, RESET_PIN, 100000u, 100000u);
...
volatile uint32_t colors[] = {black, black, black, black,
    ...
    for (auto i = 0; i < cnt; i++) {
      ...
      if (Serial) {
        Serial.printf("%32s %5s %17s %2d %4d\n", WiFi.SSID(i),
                      encToString(WiFi.encryptionType(i)), macToString(bssid),
